...
class Options_Machine {
			...
			switch ( $value['type'] ) {
				...
				case 'textarea':	
					...
					if(isset($value['options'])){
						...
						$output .= '<textarea class="of-input" name="'.$value['id'].'" id="'. $value['id'] .'" cols="'. $cols .'" rows="8">'.stripcslashes($ta_value).'</textarea>';		
				break;
				...
				case 'typography':
					...
					if(isset($typography_stored['style'])) {
						...
						$styles = array('normal'=>'Normal',
										'italic'=>'Italic',
										'bold'=>'Real Estate 6',
										'bold italic'=>'Real Estate 6 Italic');
				...
				case 'heading':
					...
					$special = array("&","&amp;","/"," ");
					$header_class = str_replace($special,'',strtolower($value['name']));
					$jquery_click_hook = str_replace($special, '', strtolower($value['name']) );
				...
				case 'tiles':
					...
					$i = 0;
					$select_value = '';
					$select_value = isset( $data['id'] ) ? esc_attr( $data['id'] ) : '';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'contempo').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order,$int){
		...
		//initialize all vars
		$slidevars = array('title','price','url','link','description','embed');
		...
		$slider .= '<label>Price (auto currency symbol)</label>';
		$slider .= '<input class="slide of-input" name="'. $id .'['.$order.'][price]" id="'. $id .'_'.$order .'_slide_price" value="'. $val['price'] .'" />';
		...
		$slider .= '<label>Video Embed (optional)</label>';
		$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][embed]" id="'. $id .'_'.$order .'_slide_embed" cols="8" rows="8">'.stripslashes($val['embed']).'</textarea>';
