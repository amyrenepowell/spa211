	...
	class AQ_Twitter_Block extends AQ_Block {
		...
		function check_plugin() {
			if(!is_plugin_active( 'twitter-widget-pro/wp-twitter-widget.php')) {
				echo __('Sorry, this block requires the <a href="http://wordpress.org/extend/plugins/twitter-widget-pro/">Twitter Widget Pro</a> plugin to be installed & activated. Please install/activate the plugin before using this block', 'framework');
				return false;
			}
			return true;
		}
		...
		function form($instance) {
			...
			<p>
				...
				<select id="<?php echo $this->get_field_id( 'avatar' ); ?>" name="<?php echo $this->get_field_name( 'avatar' ); ?>">
					...
					<option value="original"<?php selected( $instance['avatar'], 'original' ) ?>><?php _e( 'Real Estate 6al', $this->_slug ); ?></option>
				</select>
			</p>
			<p>
